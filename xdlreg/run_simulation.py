...
# %% Run simulation  o >><< o >><< o >><< o >><< o >><< o >><< o >><< o >><< o >><< o >><< o >><< o >><< o


def run_simulation(path: str = None, n_samples: int = 2000, uniform: bool = True,
                   target_bias: float = None, growth_mode: str = "human",
                   epochs: int = 25, plot_n_heatmaps: int = 20):
    """Run the LRP simulation study on pumpkins."""
    ...
    model_name = train_simulation_model(pumpkin_set=get_pumpkin_set(
        n_samples=n_samples, uniform=uniform, age_bias=target_bias, growth_mode=growth_mode),
        epochs=epochs)
